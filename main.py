...
def parse_data(html):
    ...
    prompt = (
        f"你是一个数据提取小助手，能够从一大段新闻文本中提取有用的信息并以JSON格式返回。{html},"
        '请从上面的文本中提取有用信息，返回数据格式如下：{"title": "新闻标题", "content": "新闻内容", "author": "新闻作者", "publish_time": "发布时间"}'
    )
    print(prompt)
    data = {"model": "gpt-4-all", "messages": [{"role": "user", "content": prompt}]}
...
if __name__ == "__main__":
    # url = "https://juejin.cn/"
    with open("files/test_html/去除注释.html", "r", encoding="utf-8") as f:
        html = f.read()
